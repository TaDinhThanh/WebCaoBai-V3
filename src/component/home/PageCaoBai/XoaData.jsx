...
import { ajaxCallGet, ajaxCallDelete, ajaxCallPut, getDataIdHaveUrlGoogle, getDataIdHaveVideo } from "../../Ajax";
...
const XoaData = () => {
    ...
    const handleResetKey = idKey => {
        if (data_current_id_cam) {
            ajaxCallPut('reset-key/' + idKey).then(rs => {
            ...
            if (checkKey.length > 0) {
                ...
                for (const checkbox of document.querySelectorAll('input[name="key"]')) {
                    if (checkbox.checked) {
                        ...
                        await ajaxCallDelete('delete-url-by-id-key/' + id_key).then(rs => {
                ...
                setTimeout(() => {
                    getDataIdHaveVideo(data_current_id_cam).then(rs => {
                        console.log(rs);
                        dispatch(changeDataKeyHaveVideo([...rs]))
                    })
                    getDataIdHaveUrlGoogle(data_current_id_cam).then(rs => {
                        console.log(rs);
    ...
    return (
        <div className='col-2 delete' style={{ marginLeft: '12px' }}>
            <button
                type='button'
                onClick={() => {
